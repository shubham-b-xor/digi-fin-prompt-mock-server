...
const mockResponses: Record<string, any> = {
    ...
    unstructuredFailed: {
        ...
        erroredProperties: [
            ...
            "Inadequate Data Privacy Language",
            "Non-compliant Formatting",
            "Incorrect or Outdated Clauses"
        ],
        errorMessage: customResponse['unstructured-failed']
    },
    csvFailed: {
        errorCode: 2,
        message: null,
        erroredProperties: [
            "Transaction_ID", "Date", "Account_Number", "Amount", "Currency", "Transaction_Type"
        ],
        errorMessage: customResponse['csv-failed']
    },
};
...
app.post('/submitPrompt', upload.array('files'), (req: Request, res: Response) => {
    ...
    if (Array.isArray(req.files) && req.files.length > 0) {
        mimetype = (req.files[0] as Express.Multer.File).mimetype;
    }
